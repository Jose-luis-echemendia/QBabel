...
import { useCallback, useRef } from "react"; // Added useCallback and useRef
import { Link } from "react-router-dom";

export const Books = () => {
  // Assuming 'books' is the array, and 'loading' & 'next' are siblings in state.book
  // If your state.book.books is an OBJECT like { books: [], loading: true, next: '...' }
  // then this line is correct, but you'd map `books.books` later.
  // For clarity, I'm proceeding as if state.book has { books: [], loading, next } directly.
  const { books, loading, next } = useAppSelector((state) => state.book);
  ...
  const lastBookElementRef = useCallback(
    (node) => {
      ...
      observer.current = new IntersectionObserver((entries) => {
        // entries[0] is the observed element (last book)
        if (entries[0].isIntersecting && next) {
          // console.log("Last book visible, fetching more...");
          handleFetchMoreBooks(next); // Your hook function to fetch more
        }
      });

      if (node) observer.current.observe(node); // Observe the new last element
    },
    [loading, next, handleFetchMoreBooks] // Dependencies for useCallback
  );

  // Initial loading state (when books array is empty or not yet populated)
  ...
  // No books found (after initial load attempt, and books array is still empty)
  if (!loading && (!books || books.length === 0)) {
    return (
      <div className="col-span-8 grid grid-cols-2 w-full h-fit py-3 px-8 gap-9 mt-5 ">
        <div className="col-span-2 text-center text-gray-500">
  ...
  return (
    <>
      <div className="col-span-8 grid grid-cols-2 w-full h-fit py-3 px-8 gap-9 mt-5">
        {/* Ensure 'books' here is the array of book items */}
        {books &&
          books.map((book, index) => {
            ...
            return (
              <Link
                to={`/books/${book.uid}`}
                key={book.uid}
                ref={isLastElement ? lastBookElementRef : null} // Attach ref to the last element
      ...
      {/* Show this if 'loading' is true AND there are already books displayed */}
      {loading && books && books.length > 0 && (
        <div className="col-span-10 py-5">
          <div className="flex flex-col gap-1.5 justify-center items-center ml-[350px]">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
            ...
            <span className="ml-2 text-gray-500">Cargando más libros...</span>
          </div>
        </div>
      )}

      {/* No more books to load indicator */}
      {/* Show this if 'next' is null/falsy AND there are books displayed */}
      {!next && books && books.length > 0 && (
        <div className="col-span-7 text-end py-5 text-gray-500 mr-20">
