...
const AdminCategoriesView = () => {
  ...
  const renderRow = ({ item, index, totalItems }) => {
    const { image_details, name, type, is_active, created_at } = item;
        ...
        <tr key={name} className="hover:bg-gray-100 ">
          <td className={classes}>
            <div className="flex items-center gap-3 ml-2.5">
              <Avatar src={image_details.image} alt={name} size="sm" />
              <Typography
                variant="small"
                color="blue-gray"
          ...
          <td className={classes}>
            <div className="flex gap-2.5">
              <svg
                ...
                viewBox="0 0 24 24"
                fill="currentColor"
                className="size-6 text-black hover:text-gray-800 cursor-pointer"
                onClick={() => {
    ...
    <>
      ...
      >
        <OverViewCategory
          ...
          handleOpen={() => setOpenOverViewCategoryModal(false)}
        />
      </CustomModal>
      {/* Modal de delete Category */}
      <CustomModal
        open={opendeleteCategoryModal}
        handleOpen={() => setdeleteCategoryModal(false)} // Cierra el modal
        classNameDialog="custom-dialog-class" // Clases personalizadas
        classNameBody="custom-body-class"
        exitButton={true}
        size="md"
      ...
      <CustomTable
        ...
        addItem={true}
        handleAddItem={() => (
          setSelectedCategory(null), setOpenOverViewCategoryModal(true)
        )}
        TABS={TABS}
